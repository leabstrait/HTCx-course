...
(define (contains-mcgill? los)
  (contains-team? "McGill" los))

;; String (listof String) -> Boolean
;; produce true if los includes supplied team name

(check-expect (contains-team? "UBC" empty) false)
(check-expect (contains-team? "UBC" (cons "McGill" empty)) false)
(check-expect (contains-team? "UBC" (cons "UBC" empty)) true)
(check-expect (contains-team? "UBC" (cons "McGill" (cons "UBC" empty))) true)
(check-expect (contains-team? "Toronto" (cons "UBC" (cons "McGill" empty))) false)

(define (contains-team? teamname los)
  (cond [(empty? los) false]
        [else
         (if (string=? (first los) teamname)
             true
             (contains-team? teamname (rest los)))]))
...
(check-expect (map2 sqrt (list 16 4)) (list 4 2))
(check-expect (map2 abs (list -6 4)) (list 6 4))
(check-expect (map2 string-length (list "A" "ab" "abc")) (list 1 2 3))
...
;; given a predicate and (list n0 n1 ...) return a list of ni for which (predicate ni) is true

(check-expect (filter2 positive? empty) empty)
(check-expect (filter2 positive? (list 3 -4 5)) (list 3 5))
(check-expect (filter2 even?     (list 3 1 2 4 5 7 8 9 5)) (list 2 4 8))
